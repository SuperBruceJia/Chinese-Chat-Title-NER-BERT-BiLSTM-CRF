...
def strage_combined_link_org_loc(tokens, tags):
    ...
    def print_output(data, type):
        ...
        final_result = line[1:]
        print('Stage One: Find the -> Title <- in the paragraph: ', final_result)
        ...
        # 将所有识别到的名称给合并，比如说['玉龙', '老张'] 合并为 ['玉', '龙', '老', '张']
        concat_name = []
        for i in range(len(line[1:])):
            concat_name += list(line[1:][i])
        # print(concat_name)

        # 将输入的人名给分词为单个字，例如['贾舒越'] 分词为 ['贾'], ['舒'], ['越']
        print('Please Input -> Name of the Person <- : ')
        name = str(input())
        ...
        if timer == 0:
            print('There is no match for ', name)

        else:
            for i in range(len(final_result)):
                final_result_cut = []
                for j in range(len(final_result[i])):
                    final_result_cut += list(final_result[i][j])

                for k in range(len(final_result_cut)):
                    if key == final_result_cut[k]:
                        final_key_result = final_result[i]
                        print(final_key_result, ' is ', name)
